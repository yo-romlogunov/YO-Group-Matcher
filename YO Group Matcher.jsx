...
add_null_objects_button.text = "Add Null Objects"; // Можно заменить на иконку, если есть ресурс
add_null_objects_button.preferredSize.width = 140;
...
function unlinkSelectedLayersFromGroup() {
    ...
    // Обновляем заголовки всех групп после удаления префиксов
    for (var j = 0; j < layerGroups.length; j++) {
        updateLayerGroupPanelTitle(layerGroups[j]);
        // Принудительно уведомляем панель, что нужно перерисовать себя:
        layerGroups[j].panel.notify("onDraw");
    }
...
function getGroupLayerAndCompCounts(prefix) {
    var comps = getAllCompositions();
    var layerCount = 0;
    var compCount = 0;
    for (var i = 0; i < comps.length; i++){
         var comp = comps[i];
         var compHasGroup = false;
         for (var l = 1; l <= comp.numLayers; l++){
             var layer = comp.layer(l);
    ...
    return { layers: layerCount, comps: compCount };
}

function updateLayerGroupPanelTitle(groupData) {
    var counts = getGroupLayerAndCompCounts(groupData.prefix);
    var title = groupData.name + " [" + groupData.prefix + "]";
    if (counts.layers > 0) {
        title += " | " + counts.layers + " layer" + (counts.layers > 1 ? "s" : "");
    }
    if (counts.comps > 0) {
        title += " | " + counts.comps + " Comp" + (counts.comps > 1 ? "s" : "");
    }
    groupData.panel.text = title;
}
...
function createLayerGroupUI(groupName, prefix, labelColorIndex, disableLabelColor, guideCheckboxValue, lockCheckboxValue, disableVolumePresets) {
    ...
    updateLayerGroupPanelTitle(groupData);
